...
/*
 * VFS core init only.  Filesystem backends (kernel/fs/backend/) and the
 * buffer cache (kernel/fs/backend/bcache/) self-register via their own
 * subsys inits at SUBSYS_LEVEL_BLOCK / SUBSYS_LEVEL_FS_BACKEND; the VFS
 * core never names them.  Subsystem registration itself lives in
 * fs_service.c, next to the service tables.
 */
void vfs_init(void)
{
...
long vfs_file_mmap(struct file *file, uint64_t *addr, size_t length,
		   loff_t offset)
{
	if (!file || !file->f_op || !file->f_op->mmap)
		return -ENODEV;

	return file->f_op->mmap(file, addr, length, offset);
}
