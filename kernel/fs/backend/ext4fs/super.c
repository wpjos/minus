...
static int ext4fs_backend_init(void)
{
...
/*
 * Self-registration: filesystem backends register themselves at
 * SUBSYS_LEVEL_FS_BACKEND, after the VFS core (SUBSYS_LEVEL_FS) and the
 * block layer (SUBSYS_LEVEL_BLOCK).  The VFS core never names a backend.
 */
subsys_register(ext4fs, SUBSYS_LEVEL_FS_BACKEND, ext4fs_backend_init);
