...
static long fb0_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FBIOGET_INFO:
		...
		if (copy_to_user((user_addr_t)arg, &req, sizeof(req)) != 0)
			return -EFAULT;
		return 0;
	case FBIO_FLUSH:
...
static long fb0_mmap(struct file *file, uint64_t *addr, size_t length,
		     loff_t offset)
{
	struct fb_info *info = (struct fb_info *)file->f_private;
	cap_t vs_cap = mm_current_vspace_cap();
	uint64_t uva;
	size_t size;
	int ret;
	...
	ret = mm_call(map_user_pages, vs_cap, info->phys_base, size,
